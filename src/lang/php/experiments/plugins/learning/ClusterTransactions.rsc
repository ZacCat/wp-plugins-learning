...
lrel[real, str, int] unBinDistWeightedBMN( list[int] q, Cluster[&T <: num] C, Key key )
{
	lrel[ real, num, list[int]] NN = [];
	...
	for( <s, w>  <- C, q <= s )
	{
		...
		NN += <size((q - s) + (s - q)) + 0.0, w, s >;
	}
		
	return unBinpredictNN(NN, key, q);
}

/* Predict the probability of each feature */
lrel[real, str, int] unBinpredictNN( lrel[real d, num w, list[int] V]  M, Key key, list[int] q)
{
	real pThres = 0.2;
	...
	map[int, real] ret = ();
	for( <d, w, V> <- M, s := sim(maxD, minD, d, w + 0.0, 1.0))
	{
		for( n <- V )
		{
			if(n in ret) ret[n] += s;
